...
class LT_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return sample1, sample2, target, index
...
class Places_LT(DataLoader):
    def __init__(self,  data_dir="", batch_size=60, num_workers=40, training=True, train_txt = "./data_txt/Places_LT_v2/Places_LT_train.txt",
        ...
        train_dataset = LT_Dataset(data_dir, train_txt, transform=[train_trsfm, train_trsfm_augmentation_randncls])
        if training:
            dataset = LT_Dataset(data_dir, train_txt, transform=[train_trsfm, train_trsfm_augmentation_randncls])
            eval_dataset = LT_Dataset_Eval(data_dir, eval_txt, transform=test_trsfm, class_map=train_dataset.class_map)
        else:
            dataset = LT_Dataset_Eval(data_dir, test_txt, transform=test_trsfm, class_map=train_dataset.class_map)
            eval_dataset = None
